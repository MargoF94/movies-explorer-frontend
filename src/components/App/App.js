...
function App() {
  ...
  function filterBySearchWord(list, word, isShortOn) {
    if (list.length > 0) {
      if (isShortOn === true) {
        return list.filter((movie) => movie.nameEN !== 'null' ? (movie.nameEN.toLowerCase().includes(word.toLowerCase()) && movie.duration < 40) : false);
      } else {
        return list.filter((movie) => movie.nameEN !== 'null' ? movie.nameEN.toLowerCase().includes(word.toLowerCase()) : false);
      }
    } else {
      return [];
    }
